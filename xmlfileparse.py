...
def xml_parser_xpath():
    ...
    for i in range(0, length_xpath_stories):
        final_list.append(xpath_title[i].text + " " + xpath_description[i].text)
    
    return final_list

xml_text_as_list = xml_parser_xpath()
print(xml_text_as_list)


# Alternate method using the less than ideal for loop nesting.
def xml_parser_nested_loop():
...
print_variable_from_nested_method = xml_parser_nested_loop()
print(print_variable_from_nested_method)

"""
Sample output below. Click the "raw" button on the top right to view the output as line wrapped.
